...
public class CalcuateStopsByExactly implements CalculateInter {
     ...
     * the exactly stops
     */
    private int          exactlyNumber = 0;



    public CalcuateStopsByExactly() {
        this.dbManager = DataFactory.getDbManager();
    }



    public CalcuateStopsByExactly(int pExactlyNumber) {
        this();
        this.exactlyNumber = pExactlyNumber + 1;
    }
    ...
    private void dfs(final int pCurrent, final int pEnd, final String pTemp) {
        ...
        if (pTemp.length() > this.exactlyNumber) {
            return;
        }
        if ((pCurrent == pEnd) && (pTemp.length() == this.exactlyNumber)) {
